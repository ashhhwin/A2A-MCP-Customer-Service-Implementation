...
async def handle_customer_intent(intent: str, payload: dict):
    """Async handler for each customer data intent."""
    if intent == "get_customer_info":
        return agent.invoke_tool("get_customer", normalize_payload(payload))

    elif intent == "list_customers":
        return agent.invoke_tool("list_customers", payload)

    elif intent == "update_email":
        # MCP expects 'data' key, map from 'updates' if present
        updates = payload.get("updates", {})
        return agent.invoke_tool("update_customer", {"customer_id": payload["customer_id"], "data": updates})

    elif intent == "update_customer":
        return agent.invoke_tool("update_customer", payload)

    elif intent == "get_customer_history":
        return agent.invoke_tool("get_customer_history", normalize_payload(payload))
